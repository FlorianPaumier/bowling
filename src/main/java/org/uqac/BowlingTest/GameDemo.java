...
/**
 * Range tir 1 => 10
 * tant que tir <= 9 2 tir max
 * Si 10 pins = strike donc 1 tir
 * Si second tir
 * 	pins restant tomber = spare
 * 	sinon rien
 * sinon
 * 	Si strike 2 tirs supplémentaire
 * 	Si spare second tir 3eme tir
 * 	Sinon 2 tir max
 *
 *	Si pins abatu < 10 = 1 pins == 1 point
 *	Si spare
 *		10 + pins abatu lors du prochain lancer
 *	sinon strike
 *		10 + pins du tours
 *		Si Strike
 *
 *	First row = Nb lancé
 *	Second row = pins down
 *	Third row = score
 *
 */